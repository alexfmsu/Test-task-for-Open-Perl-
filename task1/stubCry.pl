...
sub processClientReq {
    ...
    for my $_dt (@dt_arr) {
        ...
        my $obj;
        eval {
            $obj = $clientBuf{$c}[0]->incr_parse();
            1;
        } or do {
            my $msg = "wrong json <$_dt>: $@";
            chomp($msg);
            $msg =~ s/\n/\\n/g;
            logg $msg;
    ...
    for my $cp ( @{ $$obj{coupons} } ) {
        ...
        for my $p ( @{ $$obj{items} } ) {
            ...
            push @discs,
                {
                num      => $$p{num},
                discount => $$p{cost} * $$p{amount} * $rate / 100
                    + $$p{discount},
                code => $opt{aId}
                };
        }
...
sub close_onFile {
    my ($code) = @_;
    $code =~ /^cl(b|\d\d)(\d\d\d)(\d\d\d)(\d+)/
        or logg("close <$code> failed"), return;
    my $iCode = "$1-$2-$3-$4";
    logg "coupon $iCode closed";
    my $F;
    open $F, '>>', "cps/$iCode.data"
        or logg("close-open <$iCode> failed"), return;
    print $F "closed=1";
    close $F;
}
...
$S = new IO::Socket::INET( Listen => 3, ReuseAddr => 1, LocalPort => 1212 )
    or die "bindErr: $!";
